  ...
  try {
    ...
    if (data.success) {
      ...
      guests = data.guests.map(g => ({
        ...
        time: g.attendance_time ? formatAttendanceTime(g.attendance_time) : null
      }));
      ...
      showAlert("error", `Failed to load guests: ${data.message || 'Unknown error'}`);
      guests = [];
      renderGuestList();
      updateStatistics();
    }
      ...
      (guest, index) => `
        ...
        guest.id
            ...
            <td>
                ...
                ${guest.email ? `<br><small class="text-muted"><i class="fas fa-envelope"></i> ${guest.email}</small>` : ''}
            </td>
            <td>
            ...
            <td>
                <div class="btn-group btn-group-sm">
                    ...
                    <button class="btn btn-action btn-info" onclick="viewGuestQR(${guest.id})" title="View QR">
...
async function updateStatistics() {
  try {
    const response = await fetch(`/api/events/${currentEventId}/attendance-stats`);
    const data = await response.json();
...
async function downloadGuestQR() {
  ...
  try {
    const response = await fetch(`/api/guests/${currentGuestForQR.id}/qr-download`);
...
function onScanError(errorMessage) {
  // Handle scan errors silently (they happen frequently during normal scanning)
}
...
async function processQRCode(qrData) {
  try {
    ...
    if (data.success) {
      ...
      const guest = guests.find(g => g.qr_code === qrData);
      if (guest) {
        guest.attended = true;
    ...
    showScanResult(
      "error",
      "Error",
      "Failed to process QR code scan"
    );
  }
}
...
function downloadAttendance(type) {
  ...
  filteredGuests.forEach((guest, index) => {
    csv += `${index + 1},"${guest.name}","${guest.email || ''}","${guest.phone || ''}",${guest.qr_code},`;
